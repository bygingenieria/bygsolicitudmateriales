...
export default function PanelEspecialesPage() {
  ...
  const procesarSolicitud = async (id: number, estado: string) => {
    try {
      ...
      toast.success(
        estado === "RequiereCompra"
          ? "Solicitud aprobada y enviada a Compras"
          : "Solicitud Rechazada",
      );
      cargarDatos();
    } catch {
          ...
          <div className="flex items-center gap-4">
            <Link href="/dashboard/bodeguero">
              <Button
                variant="outline"
                size="icon"
                className="rounded-full shadow-sm border-orange-200 hover:bg-white"
              >
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 flex items-center gap-3">
                <PackagePlus className="w-8 h-8 text-orange-600" /> Solicitudes
                Especiales
              </h1>
              <p className="text-slate-500">
                Evaluación de materiales Nuevos o sin Stock
              </p>
            </div>
          </div>
          ...
          <CardContent className="p-0">
            <Table>
              <TableHeader className="bg-orange-50/50">
                <TableRow>
                  <TableHead className="font-bold text-slate-700 pl-6">
                    Folio
                  </TableHead>
              ...
              <TableBody>
                {cargando ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-24">
                      ...
                      <p className="text-slate-400 font-medium">
                        No hay solicitudes especiales pendientes de filtro.
                      </p>
                    </TableCell>
                  </TableRow>
                ) : (
                  solicitudes.map((s) => (
                    ...
                    >
                      <TableCell className="font-bold text-orange-600 pl-6">
                        #{s.folio}
                      </TableCell>
                      <TableCell>
                        {s.tipoPedido === "NUEVO" ? (
                        ...
                        {s.estado === "Pendiente" ? (
                          <div className="flex justify-end gap-2">
                            <Button
                              ...
                              onClick={() =>
                                procesarSolicitud(s.id, "RequiereCompra")
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                procesarSolicitud(s.id, "Rechazada")
                              }
                            >
                          ...
                          <Badge
                            variant="secondary"
                            className="opacity-50 uppercase text-[9px] font-bold"
                          >
                            {s.estado}
                          </Badge>
                        )}
