...
export default function PanelSolicitudesEspecialesPage() {
  ...
  const procesarSolicitud = async (id: number, estado: string) => {
    try {
      ...
      toast.success(estado === "RequiereCompra" ? "Solicitud enviada a Compras" : "Solicitud Rechazada");
      cargarDatos();
    } catch {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-orange-200 pb-6">
          <div className="flex items-center gap-4">
            <Link href="/dashboard/bodeguero">
              <Button variant="outline" size="icon" className="rounded-full hover:bg-white shadow-sm border-orange-200">
                <ArrowLeft className="w-5 h-5 text-orange-600" />
              </Button>
            </Link>
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 flex items-center gap-3">
                <PackagePlus className="w-8 h-8 text-orange-600" /> Panel de Especiales
              </h1>
              <p className="text-slate-500">Evaluación de materiales Nuevos o sin Stock</p>
            </div>
          </div>
          <Badge className="bg-orange-100 text-orange-700 border-orange-200 px-4 py-1 text-sm font-bold animate-pulse">
            ...
            <Table>
              <TableHeader className="bg-orange-50/50">
                <TableRow>
                  ...
                  <TableHead className="text-right pr-6 font-bold text-slate-700">Acciones de Aprobación</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {cargando ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-24">
                      ...
                      <p className="text-slate-400 font-medium">No hay solicitudes especiales pendientes por ahora.</p>
                    </TableCell>
                  </TableRow>
                ) : (
                  solicitudes.map((s) => (
                    <TableRow key={s.id} className="hover:bg-orange-50/30 transition-colors">
                      ...
                      <TableCell>
                        {s.tipoPedido === "NUEVO" ? (
                          <Badge className="bg-purple-100 text-purple-700 border-purple-200 text-[10px]">TOTALMENTE NUEVO</Badge>
                        ) : (
                          <Badge className="bg-orange-100 text-orange-700 border-orange-200 text-[10px]">STOCK AGOTADO</Badge>
                        )}
                      </TableCell>
                        ...
                        {s.estado === "Pendiente" ? (
                          <div className="flex justify-end gap-2">
                            <Button 
                              ...
                              onClick={() => procesarSolicitud(s.id, "RequiereCompra")}
                            >
                            ...
                            <Button 
                              ...
                              onClick={() => procesarSolicitud(s.id, "Rechazada")}
                            >
                              <XCircle className="w-4 h-4 mr-1.5" /> Rechazar
                            </Button>
                          </div>
                        ) : (
                          <Badge variant="secondary" className="opacity-50 uppercase text-[9px]">{s.estado}</Badge>
                        )}
                      </TableCell>
                    </TableRow>
                  ))
